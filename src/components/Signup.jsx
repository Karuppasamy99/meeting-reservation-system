...
const Signup = () => {
  ...
  const findUser = loginDetails.find((user) => user.email === email);
  console.log(findUser);
  const formHandler = (e) => {
    e.preventDefault();
    if (!findUser) {
      loginDetails.push({ name, email, password });
  ...
  console.log(JSON.parse(localStorage.getItem("current_user_details")));
  useEffect(() => {
    localStorage.setItem("current_user_details", JSON.stringify(formData));
  }, [formData]);
  return formData ? (
    <Navigate to={"/"} />
  ) : (
